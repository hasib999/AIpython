...
def search(problem,f,pop_index=0):
...
#     return search(problem,lambda node:node.path_cost)

def a_star_search(problem):
    return search(problem,lambda node:node.path_cost+heuristicSLD[node.state])
gp=graphProblem("Arad","Bucharest",graph)

def gbfs(problem):
    return  search(problem,lambda node:heuristicSLD[node.state])
gp=graphProblem("Arad","Bucharest",graph)
